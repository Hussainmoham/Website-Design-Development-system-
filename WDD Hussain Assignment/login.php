...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($passwordMatch) {
            ...
            // Try to find corresponding customer record for this user
            $customer_stmt = $conn->prepare("SELECT id FROM customers WHERE email = ? OR name = ?");
            $customer_stmt->bind_param("ss", $row["username"], $row["username"]);
            $customer_stmt->execute();
            $customer_result = $customer_stmt->get_result();
            if ($customer_result->num_rows > 0) {
                $customer_row = $customer_result->fetch_assoc();
                $_SESSION["customer_id"] = $customer_row["id"];
            }
            $customer_stmt->close();
